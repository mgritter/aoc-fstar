...
let north (p:position) : (list position) =
  match p with
  | 0 -> []
  | 1 -> []
  | 2 -> []
...
let rec distance (from:position) (to:position) : nat = 
  ...
  | (x,15) -> 1 + distance x 8
  | (x,16) -> 1 + distance x 15
  | (x,17) -> 1 + distance x 10
  | (x,18) -> 1 + distance x 17
  | (x,19) -> 1 + distance x 12
  | (x,20) -> 1 + distance x 19
  | (x,21) -> 1 + distance x 14
  | (x,22) -> 1 + distance x 21
...
let is_home_position (a:amp_state) (p:position) : bool =
  match a.color with
  | Amber -> p = 7 || p = 8 || p = 15 || p = 16
  | Bronze -> p = 9 || p = 10 || p = 17 || p = 18
  | Copper -> p = 11 || p = 12 || p = 19 || p = 20
  | Desert -> p = 13 || p = 14 || p = 21 || p = 22
...
type game_vec = (l:(list (option amp_state)){length l = 23})
...
let home_positions : list position = 
...
// These should *only* be filled up from the bottom up
// This is a more natural place to establish that constraint than
// what I did in part 1, I think.
let has_only_matching_color (v:game_vec) (a:amp_state) 
  (p1:position) (p2:position) (p3:position) (p4:position) : bool = 
  match (index v p1, index v p2, index v p3, index v p4) with
  | (None,None,None,None) -> true
  | (None,None,None,Some x) -> x.color = a.color
  | (None,None,Some y,Some x) -> x.color = a.color && y.color = a.color
  | (None,Some z,Some y,Some x) -> x.color = a.color && y.color = a.color && z.color = a.color
  // this extra case is used for is_final_position
  // where implicitly a = a2
  | (Some a2,Some z,Some y,Some x) -> x.color = a.color && y.color = a.color && z.color = a.color && a2.color = a.color 
  | _ -> false
  
let is_home_available (v:game_vec) (a:amp_state) : bool =
  match a.color with
  | Amber -> has_only_matching_color v a 7 8 15 16
  | Bronze -> has_only_matching_color v a 9 10 17 18
  | Copper -> has_only_matching_color v a 11 12 19 20 
  | Desert -> has_only_matching_color v a 13 14 21 22

let is_final_position (v:game_vec) (a:amp_state) : bool = 
  // Has to be a home position, and no later position is empty
  // i.e., it has to be one of the patterns above
  in_home_position a && is_home_available v a
...
let replace_amp (g:game_vec) (old_pos:position) (new_state:amp_state) : game_vec =
  let (before, _, after) = split3 g old_pos in
    let after_delete = before @ (None :: after) in
      ...
      assert( length (None::after) = 23 - old_pos);
      let (before, _, after) = split3 after_delete new_state.pos in
        ...
        assert( length ((Some new_state)::after) = 23 - new_state.pos );
...
let empty_or_mismatched_cost (a:amp) (s:option amp_state) (cost:nat) : nat = 
  match s with
  | None -> cost
  | Some a -> 0
  | Some _ -> cost

let empty_or_mismatched (a:amp) (v:game_vec) = 
  match a with
  | Amber -> (empty_or_mismatched_cost Amber (index v 8) 1) + 
            (empty_or_mismatched_cost Amber (index v 15) 2) + 
            (empty_or_mismatched_cost Amber (index v 16) 3)
  | Bronze -> (empty_or_mismatched_cost Bronze (index v 10) 10) + 
            (empty_or_mismatched_cost Bronze (index v 17) 20) + 
            (empty_or_mismatched_cost Bronze (index v 18) 30)
  | Copper -> (empty_or_mismatched_cost Copper (index v 12) 100) + 
            (empty_or_mismatched_cost Copper (index v 19) 200) + 
            (empty_or_mismatched_cost Copper (index v 20) 300)
  | Desert -> (empty_or_mismatched_cost Desert (index v 14) 1000) + 
            (empty_or_mismatched_cost Desert (index v 21) 2000) + 
            (empty_or_mismatched_cost Desert (index v 22) 3000)
...
let heuristic (g:game_state) : nat =
  ...
  (fold_left (fun (t:nat) o ->
   ...
   // That gets everybody to the top of their room, but we may
   // need 3 + 2 + 1 = 6 more steps
   + (empty_or_mismatched Amber v)
   + (empty_or_mismatched Bronze v)
   + (empty_or_mismatched Copper v)
   + (empty_or_mismatched Desert v)
#pop-options
  ...
  | PickNewAmp c v -> 
     List.Tot.collect (fun oa ->
         ...
         if inner_home_position a = a.pos then
            ...
            // only generate positions that have a free move
            // only move one direction in the hallway
            (collect 
              (fun n ->
                if None? (index v n) then
                [MoveAmpToHallway
...
let init_state : game_state =
  PickNewAmp 0 (let x = [
     ...
     Some ({ color=Desert; pos=8; frozen=false;finished=false;});
     Some ({ color=Amber; pos=9; frozen=false;finished=false;});
     Some ({ color=Copper; pos=10; frozen=false;finished=false;});
     ...
     Some ({ color=Desert; pos=15; frozen=false;finished=false;}); //15
     Some ({ color=Copper; pos=16; frozen=false;finished=false;});
     Some ({ color=Bronze; pos=17; frozen=false;finished=false;});
     Some ({ color=Amber; pos=18; frozen=false;finished=false;});
     Some ({ color=Amber; pos=19; frozen=false;finished=false;});
     Some ({ color=Bronze; pos=20; frozen=false;finished=false;});
     Some ({ color=Copper; pos=21; frozen=false;finished=false;});
     Some ({ color=Bronze; pos=22; frozen=false;finished=false;});
   ] in assert_norm (length x = 23); x)
...
let example_state : game_state =
  PickNewAmp 0 (let x = [
     ...
     Some ({ color=Desert; pos=15; frozen=false;finished=false;});
     Some ({ color=Amber; pos=16; frozen=false;finished=false;});
     Some ({ color=Bronze; pos=17; frozen=false;finished=false;});
     Some ({ color=Desert; pos=18; frozen=false;finished=false;});
     Some ({ color=Amber; pos=19; frozen=false;finished=false;});
     Some ({ color=Copper; pos=20; frozen=false;finished=false;});
     Some ({ color=Copper; pos=21; frozen=false;finished=false;});
     Some ({ color=Amber; pos=22; frozen=false;finished=false;});
   ] in assert_norm (length x = 23); x)
...
let print_vec (i:int) (o:option amp_state) : ML unit = 
  ...
  | Some a ->
  match a.color with
  | Amber -> print_string (if a.finished then "*" else if a.frozen then "a" else "A")
  | Bronze -> print_string (if a.finished then "*" else if a.frozen then "b" else "B")
  | Copper -> print_string (if a.finished then "*" else if a.frozen then "c" else "C")
  | Desert -> print_string (if a.finished then "*" else if a.frozen then "d" else "D")
