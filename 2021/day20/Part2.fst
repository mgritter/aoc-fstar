...
let opposite (c:cell) : cell =
  match c with
  | On -> Off
  | Off -> On
  
let rec iterate_n #w #h (rule:(list cell){length rule = 512}) (m:matrix cell w h) (n:nat) (dv:cell) (alt:bool) 
  : Tot (matrix cell w h)
    (decreases n) =
    if n = 0 then m
    else iterate_n
        rule
        (transition_matrix m rule dv)
        (n-1)
        (if alt then (opposite dv) else dv)
        alt
       
let parse_and_iterate (fn:string) (rule:string) (alt:bool): ML unit =
  ...
  let fd = open_read_file fn in
    let m = parse_matrix fd in
      ...
      | Matrix w h board ->
         print_matrix board;
         let m0 = expand_matrix board 50 in
         print_matrix m0;
         let m_final = iterate_n rule m0 50 Off alt in
             print_matrix m_final;
             print_string (sprintf "num pixels=%d\n"
                          (count_pixels m_final))

// let _ = parse_and_iterate "example.txt" example_rule false
let _ = parse_and_iterate "input.txt" input_rule true
