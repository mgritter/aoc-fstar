...
let rec last_winning_card (order:list int) (boards:list card) : ML (option (int*card)) =
  match order with
  | [] -> None
  | hd :: tl ->
    let marked_boards = mark hd boards in
      let non_winners = filter (fun b -> op_Negation (is_winner b)) marked_boards in
        if length non_winners = 0 then
           if length marked_boards = 1 then
             Some (hd, index marked_boards 0)
           else
             None
        else
           last_winning_card tl non_winners
  ...
  let fd = open_read_file fn in
    let order = parse_comma_separated_numbers (read_line fd) in
      let cs = (map to_empty_card (read_matrix_list fd)) in
        let wopt = last_winning_card order cs in
          match wopt with 
