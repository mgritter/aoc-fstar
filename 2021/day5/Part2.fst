...
type point_on_line (line:line_coords) =
   (p:point{between p.x line.x1 line.x2 /\ between p.y line.y1 line.y2})
...
let rec points_on_diagonal_line
      (line:line_coords) 
      (x_p:int{between x_p line.x1 line.x2})
      (y_p:int{between y_p line.y1 line.y2})
  :  Tot (list (point_on_line line))
     (decreases (abs (x_p - line.x2)) + (abs (y_p - line.y2))) =
  let first_point : (point_on_line line) =
      { x = x_p; y = y_p } in
  let dx = if line.x1 < line.x2 then 1 else -1 in
  let dy = if line.y1 < line.y2 then 1 else -1 in
  if x_p = line.x2 then
     [first_point]
  else if y_p = line.y2 then
     [first_point]
  else   
     first_point :: (points_on_diagonal_line line (x_p + dx) (y_p + dy))
...
let points_on_line (l:line_coords) : (list (point_on_line l)) =
  if is_horizontal l then
     ...
     points_on_diagonal_line l l.x1 l.y1
