...
let forward (d:int{d>0}) : ST unit
  (requires (fun h0 -> contains h0 horizontal))
  (ensures (fun h0 _ h1 -> (modifies (only horizontal) h0 h1)
                        /\ (sel h0 horizontal) < (sel h1 horizontal) )) =
    horizontal := !horizontal + d

let down (d:int{d>0}) : ST unit
  (requires (fun h0 -> contains h0 depth))
  (ensures (fun h0 _ h1 -> (modifies (only depth) h0 h1)
                        /\ (sel h0 depth) < (sel h1 depth) )) =
    depth := !depth + d
